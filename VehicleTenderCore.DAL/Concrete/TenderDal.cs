			...
			return (from tender in _db.Tenders
					...
					select new TenderDetailListVM()
					{
						...
						TenderAndVehicleDetailList = (from td in _db.TenderDetails
							...
							select new TenderAndVehicleDetailVM(){MinPrice = td.MinPrice,StartPrice = td.StartPrice,LicencePlate = vehicle.LicensePlate,TenderDetailId = td.Id,VehicleImages = _db.VehicleImages.Where(x=>x.VehicleId==vehicle.Id).Select(x=>x.ImagePath).ToList()}).ToList()
